#!/bin/bash
# LIMITADOR AUTO
find /etc/ElementManager/temp/RegTimeT -mmin +1440 -type f -delete >/dev/null 2>&1
[[ -e /etc/ElementManager/temp/RegTimeT ]] || {
    check_keyoficial() {
        IP=$(wget -qO- ifconfig.me)
        IP2="$IP"
        permited=$(curl -sSL "https://raw.githubusercontent.com/IntelectoDev/ElementManager/master/IP-Block")
...
echo -e "\n\033[1;31m————————————————————————————————————————————————————\n\n"
echo "/etc/ElementManager/menu.sh" >/usr/bin/menu && chmod +x /usr/bin/menu
echo "/etc/ElementManager/menu.sh" >/usr/bin/MENU && chmod +x /usr/bin/MENU
EOF
...
EOF
            ...
            echo "Actulizacion OFF" >/etc/ElementManager/temp/RegTimeT
        }
...
#BACKUP BASE DE USER
backupbase_fun() {
    find /etc/ElementManager/backuplog -mtime +7 -type f -delete
    Fecha=$(date +%d-%m-%y-%R)
    rm -rf /etc/ElementManager/backuplog/principal/*
    mkdir -p /etc/ElementManager/backuplog/principal
    cp /etc/ElementManager/cuentassh /etc/ElementManager/backuplog/principal/cuentassh
    cp /etc/ElementManager/cuentahwid /etc/ElementManager/backuplog/principal/cuentahwid
    cp /etc/ElementManager/cuentatoken /etc/ElementManager/backuplog/principal/cuentatoken
...
msg_reboot() {
    ...
    [[ -e /etc/ElementManager/temp/idtelegram ]] && {
...
reset_psoket() {
    for portdic in $(cat /etc/ElementManager/PortM/PDirect.log); do
        screen -dmS pydic-"$portdic" python /etc/ElementManager/filespy/PDirect-$portdic.py
    done
}
...
# AUTO MONITOR PROTO
resetprotos_fun() {
    tiemmoni=$(cat /etc/ElementManager/temp/T-Mon)
...
# AUTO LIMITADOR
resetlimitador_fun() {
    tiemlim=$(cat /etc/ElementManager/temp/T-Lim)
...
resetdesbloqueador_fun() {
    tiemdes=$(cat /etc/ElementManager/temp/T-Des)
...
reset_ssh() {
    ...
    # BACKUP DIARIO
    find /etc/ElementManager/temp/BackTotal -mmin +1440 -type f -delete >/dev/null 2>&1
    [[ -e /etc/ElementManager/temp/BackTotal ]] || {
 ...
 [[ -e /etc/ElementManager/temp/idtelegram ]] && {
        ...
        echo "Backup Diario Activo | $Fecha " >/etc/ElementManager/temp/BackTotal
    } &>/dev/null
...
ssh_total() {
    ...
    SSH="$(wc -l /etc/ElementManager/cuentasactivast | awk '{print $1}')"
    SSH2="$(echo ${SSH} | bc)0"
    SSH3="/10"
    echo "${SSH2}${SSH3}" | bc >/etc/ElementManager/temp/sshtotal
}
if [[ "$1" = "totallssh" ]]; then
...
contador_online() {
    ...
    while read user; do
        ...
        echo "${ONLINES}0" | bc >/etc/ElementManager/temp/Tonli
